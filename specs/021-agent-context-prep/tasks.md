...
- [x] T001 Write failing parser tests for the `[context]` attribute (bare form
...
- [x] T002 Add `AttrContext = "context"` to the attribute-kind constants in
      ...
      (ID-1 steps 3–4)
- [x] T003 Add formatter golden fixture `testdata/fmt/context-attr.rune`,
      ...
      (ID-1 steps 5–6)
- [x] T004 Commit: `feat(parser): accept the [context] task attribute`
      (ID-1 step 7)
...
- [x] T005 [US1] Write ALL seven failing `gatherContext` tests (success,
      no-hook, OS-mismatch, failure-degrade, timeout-degrade, truncation,
      masking) in `internal/cli/contextprep_test.go` per ID-3 step 1 in full —
      red-green order per constitution VI (analysis finding F2); expect
      compile failure — `gatherContext` undefined (ID-3 steps 1–2)
- [x] T006 [P] [US1] Write failing MCP tests: `TestInstructionsDelivered` /
...
- [x] T007 [US1] Create `internal/cli/contextprep.go`: `contextTask(f, goos)`
...
- [x] T008 [US1] Add `Instructions string` to `mcpserver.Options` and pass
...
- [x] T009 [US1] Run full `mcpserver` + `internal/cli` suites
...
- [x] T010 [US2] Write failing unit tests `TestBuildAgentPromptWithContext` /
...
- [x] T011 [US2] Add `buildAgentPrompt(contextText string, hasContext bool,
...
- [x] T012 [US2] Write end-to-end integration test
...
- [x] T014 [P] [US3] Checkpoint: re-run the four fault-injection tests written
      in T005 (`TestGatherContextFailureDegrades`, `TestGatherContextTimeoutDegrades`,
      `TestGatherContextTruncates`, `TestGatherContextMasksSecrets`) and
      confirm GREEN against T007's implementation; if the timeout test hangs
      ~5 s instead of degrading, stop and investigate context propagation in
      `internal/runtime` (tests moved into T005 per analysis finding F2)
- [x] T015 [P] [US3] Add integration test `TestContextHookFailureStillRunsAgent`
      in `test/integration/context_test.go`: failing hook → exit 0, notice in
      prompt, warning on stderr (ID-5 step 5, second test)
- [x] T016 [US3] Run `docker-compose run --rm test go test ./internal/cli/
...
- [x] T017 [US4] Write analyzer tests in `internal/analyzer/context_test.go`:
...
- [x] T018 [US4] Add `checkContext()` to `internal/analyzer/analyzer.go`
...
- [x] T019 [US4] Commit: `feat(analyzer): validate the [context] hook contract`
      (ID-2 step 5)
...
- [x] T020 [P] Document the attribute: `docs/GRAMMAR.md` AttrItem alternative,
...
- [x] T023 Walk quickstart.md §§2–6 manually once (author hook → CLI run →
